...
module.exports = (pool, authenticateFirebaseToken) => {
  ...
  router.post('/', async (req, res) => {
    ...
    try {
      ...
      res.status(201).json(communityRows[0]);
    } catch (error) {
  ...
  router.post('/:id/join', async (req, res) => {
    try {
      ...
      if (memberRows[0].count > 0) {
        return res.status(200).json({ message: 'Already a member of this community' });
      }
  ...
  router.post('/:id/leave', async (req, res) => {
    try {
      ...
      if (memberRows[0].count === 0) {
        return res.status(200).json({ message: 'Not a member of this community' });
      }
