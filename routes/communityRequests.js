...
module.exports = (pool, authenticateFirebaseToken) => {
    ...
    router.post('/', authenticateFirebaseToken, async (req, res) => {
      try {
        const { user_id, community_name, description, category, reason } = req.body;
        
        // Validate required fields
        if (!user_id || !community_name || !description || !reason) {
          return res.status(400).json({
            success: false,
            message: 'Missing required fields: user_id, community_name, description, reason'
          });
        }
        ...
        if (description.length > 1000 || reason.length > 1000) {
          return res.status(400).json({
            success: false,
            message: 'Description and reason must be less than 1000 characters'
          });
        }
        ...
        const [result] = await pool.execute(
          `INSERT INTO community_requests 
           (user_id, community_name, description, category, reason, status, created_at) 
           VALUES (?, ?, ?, ?, ?, 'pending', NOW())`,
          [user_id, community_name, description, category || 'General', reason]
        );
    ...
    router.get('/user/:userId', authenticateFirebaseToken, async (req, res) => {
      try {
        ...
        const [requests] = await pool.execute(
          `SELECT id, community_name, description, category, reason, status, 
